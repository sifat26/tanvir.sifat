...
// ── Small builders ───────────────────────────────────────────────────────────
const run = (text, opts = {}) => new TextRun({ text, font: FONT, size: BODY_SIZE, color: DARK, ...opts });
...
const dot = (padded = true) => run(padded ? '  ·  ' : '·', { color: MUTED, size: SMALL });
...
function richLine(parts, opts = {}) {
  const children = parts.map((p) => (Array.isArray(p) ? run(p[0], p[1] || {}) : run(p)));
...
function titleDateRow(leftParts, dates, opts = {}) {
  const children = leftParts.map((p) => (Array.isArray(p) ? run(p[0], p[1] || {}) : run(p, { bold: true })));
...
function roleLine(role) {
  return new Paragraph({
    spacing: { after: 40 },
    children: [new TextRun({ text: role, font: FONT, size: BODY_SIZE, bold: true, italics: true, color: ACCENT })],
  });
}
...
function metaLine(text, opts = {}) {
  return new Paragraph({
    ...
    children: [new TextRun({ text, font: FONT, size: SMALL, italics: true, color: MUTED })],
  });
}
...
const clientProjects = [
  ...
  },
  {
    ...
    bullets: [
      ['Built ', ['storefront and product pages', { bold: true }], ' in Next.js against an Express REST API.'],
      ['Implemented ', ['user authentication', { bold: true }], ' and ', ['session handling', { bold: true }], '.'],
      [
        'Set up ',
        ['SEO metadata', { bold: true }],
  ...
  },
  {
    ...
    bullets: [
      ...
      ['Modeled and integrated the ', ['product and user data layer', { bold: true }], '.'],
      [
        'Implemented ',
        ['authentication', { bold: true }],
        ' and the ',
...
for (const p of clientProjects) {
  children.push(titleDateRow([[p.name, { bold: true }]], p.site, { before: 140 }));
  children.push(metaLine(p.stack, { after: 30 }));
  p.bullets.forEach((b) => children.push(bullet(b)));
}
for (const p of personalProjects) {
  children.push(titleDateRow([[p.name, { bold: true }]], p.stack, { before: 140 }));
...
for (const [label, values] of skills) {
  children.push(
    new Paragraph({
      spacing: { after: 30 },
      children: [run(label + ': ', { bold: true, color: DARK }), run(values, { color: ACCENT })],
    }),
  );
}
...
const edu = [
  [
    ...
    'Bangladesh',
    'M.Sc. in Information & Communication Technology',
    '2025 – Present',
  ],
  [
    'Mawlana Bhashani Science & Technology University',
